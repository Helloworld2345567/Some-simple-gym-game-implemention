...
def train(env, agent, num_episode, eps_init, eps_decay, eps_min, max_t, num_frame=2, constant=0):
    ...
    for i in range(1, 1 + num_episode):
        ...
        while not done and t < max_t:
            ...
            if t % 3 == 0 and len(agent.memory) >= agent.bs:
                agent.learn()
        ...
        if i % 100 == 0:
            torch.save(agent.Q_local.state_dict(), './Air_results/AIRRAID_weights3.pth')
            np.save('AIRRAID_rewards3.npy', rewards_log)
            print()
...
if __name__ == '__main__':
    ...
    agent = Agent(NUM_FRAME, env.action_space.n, BATCH_SIZE, LEARNING_RATE, TAU, GAMMA, DEVICE, True)#对的
